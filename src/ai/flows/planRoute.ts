...
<<<<<<< HEAD
      ...
      if (currentChargeKm < stepDistanceKm) {
        ...
        console.warn(
          `Not enough charge to complete step. Charge: ${currentChargeKm}km, Step: ${stepDistanceKm}km`
        );
      }
      ...
      const proactiveCheckThresholdKm =
        vehicleMaxRangeKm * 0.2 + safetyBufferKm;
      ...
      if (currentChargeKm - stepDistanceKm < proactiveCheckThresholdKm) {
        ...
        if (bestStation) {
          ...
          if (!requiredChargingStops.some((s) => s.id === bestStation.id)) {
            ...
            const chargeNeededTo80PercentKwh =
              vehicle.batteryCapacity * 0.8 -
              (currentChargeKm - stepDistanceKm) / vehicle.consumption;

            if (chargeNeededTo80PercentKwh > 0) {
              // Time (h) = Energy (kWh) / Power (kW)
              const chargingTimeHours =
                chargeNeededTo80PercentKwh / bestStation.power;
          ...
          console.warn(
            "Could not find any available charging stations near",
            searchPoint
          );
        }
      }
    ...
    const totalDurationSeconds =
      totalDriveDurationSeconds + totalChargingTimeSeconds;
=======
        const stepDistanceKm = (step.distance?.value || 0) / 1000;
