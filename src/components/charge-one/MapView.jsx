...
export default function MapView({
    ...
    (lat, lng, radius) => {
      findStations({ latitude: lat, longitude: lng, radius })
        ...
        .catch((err) => {
          ...
          toast({
            variant: "destructive",
            title: "Could not find nearby stations.",
          });
        });
    },
  ...
  const activeStation =
    activeMarker && "name" in activeMarker.content
      ? activeMarker.content
      : null;
  const activeTitle =
    activeMarker && "title" in activeMarker.content
      ? activeMarker.content.title
      : null;
    ...
    >
      {isLoaded && (
        <>
          ...
          {isNavigating && currentLocation && (
            <MarkerF position={currentLocation} icon={navigationArrowIcon} />
          )}
